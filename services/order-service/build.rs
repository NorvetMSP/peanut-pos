...
fn main() {
    if cfg!(target_os = "windows") {
        if let Some(dir) = find_rdkafka_vcpkg_zlib_dir() {
            ...
            println!("cargo:warning=order-service linking zlib from {}", dir.display());
        } else {
            println!("cargo:warning=order-service could not locate zlib import library; falling back to static=z");
