//! Negative test: ensure no capture occurs when TEST_CAPTURE_KAFKA is NOT set
...
#[tokio::test]
async fn void_payment_no_capture_when_env_absent() {
    // Ensure env vars are UNSET
    std::env::remove_var("TEST_CAPTURE_KAFKA");
    std::env::set_var("TEST_KAFKA_NO_BROKER","1"); // still bypass actual broker
    // Drain any prior captured events
    let _ = test_support::take_captured_payment_voided();
    ...
    let captured = test_support::take_captured_payment_voided();
    assert!(captured.is_empty(), "expected no captured events, got {:?}", captured);
}
