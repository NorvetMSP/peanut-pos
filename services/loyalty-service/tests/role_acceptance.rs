use loyalty_service::LOYALTY_VIEW_ROLES;
...
#[test]
fn cashier_role_allowed_for_loyalty_view() {
    let ctx = mk_ctx(Role::Cashier);
    ensure_any_role(&ctx, LOYALTY_VIEW_ROLES).expect("cashier should be allowed for loyalty view");
}

#[test]
fn superadmin_role_allowed_for_loyalty_view() {
    let ctx = mk_ctx(Role::SuperAdmin);
    ensure_any_role(&ctx, LOYALTY_VIEW_ROLES).expect("superadmin should be allowed for loyalty view");
}