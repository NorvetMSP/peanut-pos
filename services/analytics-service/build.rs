...
fn main() {
    if cfg!(target_os = "windows") {
        if let Some(dir) = find_rdkafka_vcpkg_zlib_dir() {
            ...
            println!("cargo:warning=analytics-service linking zlib/zstd from {}", dir.display());
        } else {
            println!("cargo:warning=analytics-service could not locate zlib import library; falling back to static=z");
