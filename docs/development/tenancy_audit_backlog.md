...
| TA-FND-4 | Cross-service unified HTTP error envelope | FND | In-Progress | TA-FND-3 | Shared crate in place. Product, Inventory, Order services migrated (all handlers return ApiError). Unified error JSON + X-Error-Code header consistent. Error metrics middleware now live in all three (order added). Kafka feature gating applied to Inventory, Product, Order to reduce Windows build friction. Assertion macro available; partial adoption (next rollout: payment, loyalty, customer, integration-gateway). Remaining to mark Done: migrate remaining HTTP services to ApiError + add metrics layer + adopt assertion macro in their tests. |
