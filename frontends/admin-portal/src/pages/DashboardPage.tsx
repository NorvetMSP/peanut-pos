...
const DashboardPage: React.FC = () => {
  ...
  const fetchSummary = useCallback(async (headers: Record<string, string>): Promise<SummaryResponse> => {
    const response = await fetch(`${ANALYTICS_SERVICE_URL}/analytics/summary`, {
      headers,
    });
    if (!response.ok) {
  ...
  const fetchAnomalies = useCallback(async (headers: Record<string, string>): Promise<string[]> => {
    const response = await fetch(`${ANALYTICS_SERVICE_URL}/analytics/anomalies`, {
      headers,
    });
    if (!response.ok) {
  ...
  const fetchProducts = useCallback(async (headers: Record<string, string>): Promise<ProductNameMap> => {
  ...
  const loadData = useCallback(
    async (opts?: { initial?: boolean }) => {
      ...
      try {
        const [summaryResult, anomaliesResult, productsResult] = await Promise.allSettled([
    ...
    [buildHeaders, ensureTenantContext, fetchAnomalies, fetchProducts, fetchSummary]
  );
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col">
      <div className="admin-section-modern">
        ...
        <div className="admin-section-content">
          ...
          {isLoading ? (
            <div className="text-center text-gray-600 dark:text-gray-300 py-8">Loading dashboard...</div>
          ) : (
            <div className="grid gap-6">
              ...
              <div className="rounded-lg bg-white dark:bg-gray-800 p-6 shadow">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Top 5 Products</h3>
                {topItems.length === 0 ? (
                  <p className="mt-3 text-sm text-gray-500 dark:text-gray-300">No sales yet today.</p>
                ) : (
                  <ul className="mt-3 space-y-2">
                    {topItems.map(item => {
                      const label = productNames[item.product_id] ?? item.product_id;
                      return (
                        <li
                          key={item.product_id}
