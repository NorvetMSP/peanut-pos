...
const ReturnsPageContent: React.FC = () => {
  ...
  const [orderDetail, setOrderDetail] = useState<OrderDetailRecord | null>(
    null,
  );
  ...
  const tenantId = useMemo(
    () => (currentUser?.tenant_id ? String(currentUser.tenant_id) : null),
    [currentUser],
  );
  ...
  const fetchReturns = useCallback(async () => {
    ...
    try {
      ...
      if (historyFilters.orderId.trim())
        params.set("order_id", historyFilters.orderId.trim());
      if (historyFilters.storeId.trim())
        params.set("store_id", historyFilters.storeId.trim());
      if (historyFilters.startDate)
        params.set("start_date", historyFilters.startDate);
      if (historyFilters.endDate)
        params.set("end_date", historyFilters.endDate);

      const response = await fetch(
        `${ORDER_SERVICE_URL}/returns?${params.toString()}`,
        {
          method: "GET",
          headers: buildHeaders(),
        },
  ...
  const submitReturn = async () => {
    ...
    try {
      ...
      setSubmitError(
        err instanceof Error ? err.message : "Return submission failed.",
      );
    } finally {
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <header className="rounded-lg bg-white p-6 shadow">
          ...
          <p className="text-sm text-gray-600">
            Review prior returns and process new refunds against completed
            orders.
          </p>
        </header>
            ...
            <label className="flex flex-col text-sm font-medium text-gray-700">
              ...
              <div className="mt-1 flex gap-2">
                ...
                <button
                  ...
                  onClick={() =>
                    lookupOrderId.trim() &&
                    void loadOrderDetail(lookupOrderId.trim())
                  }
          ...
          {orderDetail && (
            <div className="mt-4 space-y-4">
              <div className="rounded border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
                <p>
                  <strong>Order:</strong> {orderDetail.order.id}
                </p>
                <p>
                  <strong>Status:</strong> {orderDetail.order.status}
                </p>
                <p>
                  <strong>Payment:</strong> {orderDetail.order.payment_method}
                </p>
                <p>
                  <strong>Total:</strong> ${orderDetail.order.total.toFixed(2)}
                </p>
              </div>
          ...
          <div className="mb-4 grid gap-4 md:grid-cols-4">
            <label className="flex flex-col text-sm font-medium text-gray-700">
              ...
              <input
                ...
                onChange={(event) =>
                  handleHistoryFilterChange("orderId", event.target.value)
                }
            ...
            <label className="flex flex-col text-sm font-medium text-gray-700">
              ...
              <input
                ...
                onChange={(event) =>
                  handleHistoryFilterChange("storeId", event.target.value)
                }
            ...
            <label className="flex flex-col text-sm font-medium text-gray-700">
              ...
              <input
                ...
                onChange={(event) =>
                  handleHistoryFilterChange("startDate", event.target.value)
                }
              />
            </label>
            <label className="flex flex-col text-sm font-medium text-gray-700">
              ...
              <input
                ...
                onChange={(event) =>
                  handleHistoryFilterChange("endDate", event.target.value)
                }
              />
            </label>
          </div>
