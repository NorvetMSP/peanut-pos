...
const isAuditEntriesContainer = (
  value: unknown,
): value is AuditEntriesContainer =>
  typeof value === "object" && value !== null && "entries" in value;

const isProductAuditEntry = (
  entry: ProductAuditEntry | null,
): entry is ProductAuditEntry => entry !== null;
...
function normalizeAuditEntry(entry: unknown): ProductAuditEntry | null {
  ...
  const actorId =
    typeof candidate.actor_id === "string" ? candidate.actor_id : null;
  const actorName =
    typeof candidate.actor_name === "string" ? candidate.actor_name : null;
  const actorEmail =
    typeof candidate.actor_email === "string" ? candidate.actor_email : null;
...
const ProductListPageContent: React.FC = () => {
  ...
  const [auditLogByProduct, setAuditLogByProduct] = useState<
    Record<string, ProductAuditEntry[]>
  >({});
  const [auditLoadingByProduct, setAuditLoadingByProduct] = useState<
    Record<string, boolean>
  >({});
  const [auditErrorByProduct, setAuditErrorByProduct] = useState<
    ...
    try {
      ...
      const normalized = Array.isArray(data)
        ? data
            ...
            .filter((item): item is ServiceProduct => Boolean(item))
        : [];
    ...
    async (productId: string, force = false): Promise<void> => {
      ...
      try {
        const response = await fetch(
          `${PRODUCT_SERVICE_URL}/products/${productId}/audit?limit=10`,
          {
            headers: buildHeaders(),
          },
        );
        ...
        setAuditErrorByProduct((prev) => ({
          ...prev,
          [productId]: "Unable to load product history.",
        }));
      } finally {
  ...
  const handleSaveEdit = async (): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(
        `${PRODUCT_SERVICE_URL}/products/${editingProductId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleToggleActive = async (product: ServiceProduct): Promise<void> => {
    ...
    try {
      const response = await fetch(
        `${PRODUCT_SERVICE_URL}/products/${product.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            ...
            >
              ...
              <div className="md:col-span-2 flex items-center gap-4">
                ...
                <span className="text-sm text-gray-600 dark:text-gray-400">
                  Preview updates automatically when you enter a URL.
                </span>
              </div>
                      ...
                      return (
                        <React.Fragment key={prod.id}>
                          <tr className="bg-gray-50 dark:bg-gray-900">
                            <td className="px-4 py-2 text-gray-900 dark:text-gray-100">
                              <div className="flex items-start gap-3">
                                <img
                                  src={
                                    editForm.image.trim().length > 0
                                      ? editForm.image.trim()
                                      : DEFAULT_IMAGE_PLACEHOLDER
                                  }
                                  alt={
                                    (editForm.name || "Product") + " preview"
                                  }
                                  className="h-16 w-16 rounded-md border border-gray-200 object-cover"
                                  onError={(event) => {
                                ...
                                <div className="flex-1 space-y-2">
                                  ...
                                  <p className="text-xs text-gray-500 dark:text-gray-400">
                                    Clear the field to revert to the default
                                    placeholder image.
                                  </p>
                                </div>
                              </div>
                          ...
                          <tr className="bg-gray-50 dark:bg-gray-900">
                            <td
                              colSpan={4}
                              className="px-4 pb-4 text-gray-900 dark:text-gray-100"
                            >
                                  ...
                                  <ul className="space-y-3 text-sm">
                                    {auditEntries.map((entry) => {
                                      ...
                                      return (
                                        <li
                                          key={entry.id}
                                          className="rounded bg-white/80 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-700 p-3"
                                        >
                                          ...
                                          <pre className="mt-2 max-h-32 overflow-auto rounded bg-gray-900/80 text-gray-100 text-xs p-2">
                                            {JSON.stringify(
                                              entry.changes,
                                              null,
                                              2,
                    ...
                    return (
                      <React.Fragment key={prod.id}>
                        <tr className="border-b border-gray-200 dark:border-gray-700">
                          ...
                          <td className="px-4 py-2 text-gray-900 dark:text-gray-100">
                            <span
                              className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${prod.active ? "bg-emerald-100 text-emerald-700" : "bg-gray-300 text-gray-700"}`}
                            >
