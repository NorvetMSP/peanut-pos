...
const STATUS_OPTIONS = ['PENDING', 'COMPLETED', 'PARTIAL_REFUNDED', 'REFUNDED', 'VOIDED'];
...
const OrdersPage: React.FC = () => {
  ...
  const viewReceipt = useCallback(
    async (orderId: string) => {
      ...
      try {
        const response = await fetch(`${ORDER_SERVICE_URL}/orders/${orderId}/receipt`, {
          method: 'GET',
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Search and inspect orders, receipts, and returns.</p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <label className="flex flex-col text-sm font-medium text-gray-700">
              ...
              <input
                ...
                onChange={(event) => handleFilterChange('customerTerm', event.target.value)}
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-8 text-center text-sm text-gray-600">
                      {error ? error : 'No orders found for the selected filters.'}
                    </td>
                  </tr>
                ) : (
                  orders.map((order) => {
                    ...
                    return (
                      <tr key={order.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-gray-700">{created.toLocaleString()}</td>
                        <td className="px-4 py-3 text-gray-700">{order.status}</td>
                        <td className="px-4 py-3 text-gray-700">{order.payment_method}</td>
                        <td className="px-4 py-3 text-right text-gray-700">${order.total.toFixed(2)}</td>
                        <td className="px-4 py-3 text-gray-700">{customerLabel}</td>
                        <td className="px-4 py-3 text-gray-700">{order.store_id ?? '--'}</td>
