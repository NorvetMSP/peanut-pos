...
import { SUPER_ADMIN_ROLES } from "../rbac";
...
const normalizeIntegrationKey = (
  entry: unknown,
): IntegrationKeyRecord | null => {
  if (typeof entry !== "object" || entry === null) return null;
...
const isIntegrationKeyCreated = (
  value: unknown,
): value is IntegrationKeyCreated => {
  if (typeof value !== "object" || value === null) return false;
  const candidate = value as IntegrationKeyJson;
  return (
    typeof candidate.api_key === "string" &&
    normalizeIntegrationKey(value) !== null
  );
};
...
const SettingsPageContent: React.FC = () => {
  ...
  const tenantHeader = currentUser?.tenant_id
    ...
    async (tenantId: string): Promise<void> => {
      ...
      try {
        const response = await fetch(
          `${AUTH_SERVICE_URL}/tenants/${tenantId}/integration-keys`,
          {
  ...
  const handleCreateKey = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `${AUTH_SERVICE_URL}/tenants/${selectedTenantId}/integration-keys`,
        {
          method: "POST",
          headers: buildHeaders(true),
          body: JSON.stringify({
      ...
      setSuccessMessage(
        "A new integration key was generated. Copy it now; it will not be shown again.",
      );
      setKeyLabel("");
  ...
  const handleRevokeKey = async (keyId: string): Promise<void> => {
    ...
    try {
      const response = await fetch(
        `${AUTH_SERVICE_URL}/integration-keys/${keyId}/revoke`,
        {
          method: "POST",
          headers: buildHeaders(),
        },
  ...
  const handleClearQueue = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(
        `${ORDER_SERVICE_URL}/orders/offline/clear`,
        {
          method: "POST",
          headers,
        },
    ...
    >
      <div className="admin-section-modern">
        <div className="admin-section-header">
          ...
          <p>
            Provision tenants and manage integration API keys on a per-tenant
            basis.
          </p>
        </div>
        ...
        <div className="admin-section-content">
          ...
          {latestKeySecret && (
            <div className="rounded bg-amber-50 border border-amber-300 text-amber-900 px-4 py-4 mb-6">
              ...
              <p className="font-mono break-all text-sm mb-3">
                {latestKeySecret}
              </p>
              <button
          ...
          <section className="rounded-lg bg-white dark:bg-gray-800 p-6 shadow">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  Generate and rotate API keys used by external systems per
                  tenant.
                </p>
              </div>
            ...
            >
              ...
              <div className="flex items-center mt-6 md:mt-0">
                <label className="inline-flex items-center text-sm text-gray-600 dark:text-gray-300">
                  <input
                    ...
                    onChange={(event) =>
                      setRevokeExisting(event.target.checked)
                    }
                  />
              ...
              <div className="md:col-span-2 flex justify-end">
                <button
                  className="admin-section-btn"
                  type="submit"
                  disabled={creatingKey || !selectedTenantId}
                >
            ...
            <div className="mt-6">
              ...
              {loadingKeys ? (
                ...
                <p className="text-sm text-gray-500">
                  No integration keys found for this tenant.
                </p>
              ) : (
